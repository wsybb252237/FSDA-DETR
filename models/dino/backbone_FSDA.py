...
from .style_rectification_FSDA  import StyleRectification
...
class BackboneBase(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, train_backbone: bool, num_channels: int, return_interm_indices: list, args_config):
        super().__init__()
        ...
        # ----style_rectification
        self.styleRectified1 = StyleRectification(num_prototype=2,
                                               channel_size=256,
                                               batch_size=self.args_config.batch_size,
                                               gamma=0.9,
                                               dis_mode='was',
                                               channel_wise=False)

        self.styleRectified2 = StyleRectification(num_prototype=2,
                                               channel_size=512,
                                               batch_size=self.args_config.batch_size,
                                               gamma=0.9,
                                               dis_mode='was',
    ...
    def forward(self, tensor_list: NestedTensor):
        # include:  'conv1', 'bn1', 'relu', 'maxpool', 'layer1', 'layer2', 'layer3', 'layer4'
        # init_layer
        init_f = self.body.conv1(tensor_list.tensors)
        init_f = self.body.bn1(init_f)
        init_f = self.body.relu(init_f)
        init_f = self.body.maxpool(init_f)

        # feature layer
        # Layer 0 - 4x downsampling
        layer0_f = self.body.layer1(init_f)  # [arg.batch_size*2,256,h,w]
        # --convert 1----
        layer0_f = self.styleRectified1(layer0_f)  # [b,256,h,w]
        # ************

        # Layer 1 - 8x downsampling
        layer1_f = self.body.layer2(layer0_f)  # [b,512,h,w]
        # --convert 2----
        layer1_f = self.styleRectified2(layer1_f)


        # Layer 2 - 16x downsampling
        layer2_f = self.body.layer3(layer1_f)  # [b,1024,h,w]

        # Layer 3 - 32x downsampling
        layer3_f = self.body.layer4(layer2_f)  # [b,2048,h,w]


        # xs = self.body(tensor_list.tensors)
        xs = {'1': layer1_f, '2': layer2_f, '3': layer3_f}
...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
        ...
        super().__init__(backbone, train_backbone, num_channels, return_interm_indices, args_config)
