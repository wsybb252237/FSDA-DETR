...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        # Debug使用，打印源域和目标域的图像加载以及标签信息等，看是否是属于同时将源域和目标域的图像和标签混合后送入模型进行前向传播
        # 此处由于samples是NestedTensor对象，所以需要将samples转换为tensor
        # size = samples.tensors.size()
        # print('目标域和源域的图像大小:', size)
        # print('目标域和源域的标签:', targets)
        # 此处的samples是将源域和目标域一个批次中同时加载2张图像并合并---NestedTensor
        samples = samples.to(device)
        # 此处的targets是将源域和目标域一个批次中加载对应的标签信息张图像并合并---list
